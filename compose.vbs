...
If (WScript.Arguments.Count = 0) then
    MsgBox "Si prega di specificare il nome del file/Please specify the file name!", vbExclamation, "Error"
    Wscript.Quit()
End if
...
Function importModulesTxt(sADPFilename, sImportpath)
    ...
    if (fso.FileExists(sADPFilename)) Then
        WScript.StdOut.Write sADPFilename & " esiste già. Eseguire l'override/already exists. Override? (y/n) "
        dim sInput
